...
/// The STWO AIR component for element-wise Exp2 (`2^x`) operations.
/// Wraps the `Exp2Eval` logic within the STWO `FrameworkComponent`.
/// Correctness of `2^x` is enforced via a lookup argument into a preprocessed table.
...
pub struct Exp2Eval {
    ...
    lookup_elements: exp2::Exp2LookupElements,
}
...
impl FrameworkEval for Exp2Eval {
    ...
    ///        effectively constraining `out_val` to be `2^input_val`.
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        // Ensure we use EXACTLY the same sign for the component side as Sin uses
        eval.add_to_relation(RelationEntry::new(
            &self.lookup_elements,
            lookup_mult.into(), // Positive for the component side
            &[input_val, out_val],
        ));
