...
impl Exp2Lookup {
    ...
    pub fn preprocessed(&self) -> HashMap<String, Vec<M31>> {
        ...
        for i in 0..num_entries {
            ...
            let input = (x * layout.input_scale).round() as i32;
            let output = ((2.0_f64.powf(x)) * layout.output_scale).round() as i32;

            lut_0.push(M31::from(input));
            lut_1.push(M31::from(output));
        }
    ...
    pub fn record_lookup(&mut self, input_val: numerair::Fixed<DEFAULT_FP_SCALE>, out_val: numerair::Fixed<DEFAULT_FP_SCALE>) {
        let layout = &self.layout;
        ...
        if multiplicities.table.len() <= index {
            multiplicities.table.resize(index + 1, Exp2LookupTraceTableRow {
        ...
        // println!("Recording lookup: input={}, output={}, index={}", x, out_val.to_f64(), index);
    }
    ...
    pub fn write_trace(
        ...
        let res = claim_generator.write_trace(tree_builder).expect("Failed to write Exp2 lookup trace");
